        ...
        rolesLoop: for (int i = 0; i < actorsForRoles.length; i++) {
            ...
            actorLoop: for (int j = 0; j < actorsForRoles[i].length; j++) {
                ...
                if (actor == 0 || actor == 1) {
                    ...
                    if (actor == 0) {
                        if (diva1Assigned) {
                            // Diva 1 is already assigned, try the next actor
                            continue actorLoop;
                        } else {
                            if (diva2Assigned) {
                                // Check that diva 2's role is not in the same scene

                                int divaRole = findIndex(assignedRoles, 1);

                                List<Integer> scenesForDiva2Role = roleInScenesMap.get(divaRole);
                                List<Integer> scenesForDiva1Role = roleInScenesMap.get(i);

                                for (Integer scene : scenesForDiva1Role) {
                                    if (scenesForDiva2Role.contains(scene)) {
                                        // Diva 2's role is in the same scene, try the next actor
                                        continue actorLoop;
                                    }
                                }
                            }
                        ...
                        if (diva2Assigned) {
                            // Diva 2 is already assigned, try the next actor
                            continue actorLoop;
                        } else {

                            if (diva1Assigned) {
                                // Check that diva 1's role is not in the same scene

                                int divaRole = findIndex(assignedRoles, 0);

                                List<Integer> scenesForDiva1Role = roleInScenesMap.get(divaRole);
                                List<Integer> scenesForDiva2Role = roleInScenesMap.get(i);

                                for (Integer scene : scenesForDiva2Role) {
                                    if (scenesForDiva1Role.contains(scene)) {
                                        // Diva 1's role is in the same scene, try the next actor
                                        continue actorLoop;
                                    }
                                }
                            }
